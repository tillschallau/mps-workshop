...
public static void readLine(){
  ...
  \begin{aufgabe}
  	...
  	\\
  	\textbf{Note:} If your desired class does not exist in your current context, you can add \textit{Accessories Models} by right-clicking onto \menu[,]{de.tudo.cs.ls14.aqua.mps.workshop.java, Module Properties}. Navigate to \menu[,]{Runtime, Accessories Models} and add the required models using the \texttt{+} button.
  \end{aufgabe}
	...
	\begin{aufgabe}
		...
		\texttt{de.tudo.cs.ls14.aqua.mps.workshop.expressions} by right-clicking onto\\ \menu[,]{mps-workshop, New, Language}. Check the \texttt{Create Sandbox Solution} in the process.
	\end{aufgabe}
	...
	For the next exercise you can checkout the branch \menu{expressions} to work with the shared solution, or work with your own concepts.

	\begin{aufgabe}
		Add editors for your concepts, such that the order of operations is clearly visible.\\
		\\
		\textbf{Note:} For some visual modifications you may need the \menu{Inspector}. To show it, click on the respective button in the lower right corner of your IDE.
	\end{aufgabe}

	For the next exercise you can checkout the branch \menu{expression-editor} to work with the shared solution, or work with your own code.
	
	\begin{aufgabe}
		Add intentions to each \texttt{AExp} and \texttt{BExp} to allow changing of the operator, without starting from scratch.
	\end{aufgabe}
